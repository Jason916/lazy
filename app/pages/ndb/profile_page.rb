...
class ProfilePage < Lol::Page
  ...
  button :create_contact_save_button, :name=> "yt1"
  #h3 text
  ...
  button :save_compliance_note1, :id=> "compliance-note-save-btn"
  #cancel compliance note 
  button :cancel_compliance_note, :id=> "compliance-note-cancel-btn"
  ...
  #click am
  link :select_am, :xpath=> "//div[@id='s2id_ClientAmPercentage_0_amid']/a"
  #select am
  div :click_am, :text=> "test_am1"
  #click second am
  link :select_second_am, :xpath=> "//div[@id='s2id_ClientAmPercentage_1_amid']/a" 
  #input test_am1
  text_field :input_test_am1, :xpath=> "//div[6]/div/input"
  #input percentage
  ...
  #click client contact rem
  link :select_contact_rem, :xpath=> "//div[@id='s2id_ClientContact_rem']/a"
  #select client contact rem
  div :click_client_contact_rem, :text=> "test_km1"
  ...
  element :edit_cashnote, :i ,:xpath=> "//div[2]/div/div[5]/a[1]/i"
  #cash content
  ...
  #cancel edit cash note
  button :cancel_edit_btn, :text=> "Cancel"
  #cancel create contact btn
  link :cancel_create_contact_btn, :xpath=> "//a[contains(text(),'Cancel')]"
  #delete cash note
  element :delete_cash_note1, :i ,:xpath=> "//div[2]/div/div[5]/a[2]/i"
  #create client contact error alert
  div :client_contact_error_alert, :class=> "alert alert-block alert-error"
  #input mobile
  text_field :input_mobile, :id=> "Contact_mobile_mobilephone"
  #mobile alert
  span :mobile_alert, :xpath=> "//div/span"
  #input area code
  text_field :input_area_code, :id=> "Contact_tel_citycode"
  #input telephone
  text_field :input_telephone, :id=> "Contact_tel_telephone"
  #click add other contact
  button :click_add_other_contact, :id=> "add_contact"
  #verify other contact info
  div :verify_other_contact_info, :xpath=>"//div[7]/div[3]/div"
  #delete client contact 
  element :delete_client_contact_link, :i ,:xpath=> "//tr[5]/td[20]/a/i"
  #delete sale note link
  element :delete_sale_note_link, :i ,:xpath=> "//div/a/i"
  #click reveal client name
  label :click_reveal_client_name, :xpath=> "//div[@id='revealname-button']/div/label"
  #click special requirement
  label :click_special_requirement, :xpath=> "//div[@id='specialrequirement-button']/div/label"
  #am alert info 
  div :am_alert_info, :id=>"error-alert"
  ...
  def add_compliance_notes #添加财务备注
      begin
        self.add_compliance_note_element.when_present.click
        self.input_client_compliance_message_element.when_present.clear
        self.input_client_compliance_message_element.when_present.append "1@_ q2%$## -#"+self.input_random_num
        rescue 
        f = File.new(File.join(path,"#{time}.txt"),"a+")
        f.puts "error:#{$!} at:#{$@}"
        f.puts "=======================================================================#{log_time}"
  ...
  def edit_account_information percentage#编辑account information
      begin
        ...
        self.click_am_element.when_present.click
  ...
  def add_am am,percentage,whether_verify_alert#在profil页面account框体中增加am
      begin
        ...
        self.select_second_am_element.when_present.click
        select_text_of_am = self.select_am_element.when_present.text#针对am唯一性的优化点
        self.input_test_am1_element.when_present.set am
  ...
  def create_contact#创建一个联系人  
      ...
      self.input_contact_mobilephone_element.when_present.append "15195771#{rand(10000)}"
      ...
      sleep 2
      self.select_contact_nps_element.when_present.click
      self.send_keys :down
      self.send_keys :enter
      self.select_contact_rem_element.when_present.click
      self.click_client_contact_rem_element.when_present.click
  ...
  def cancel_delete_compliance_notes
    message = self.confirm(false) do
      self.delete_compliance_link_element.when_present.click
    end
  end  
  ...
  def input_random_num
    rand_num= [*'a'..'z',*'0'..'9',*'A'..'Z'].sample(8).join
  end
  
  def select_cash_date
    self.send_keys :right
  end
  
  def delete_cash_note
    message =self.alert do
      self.delete_cash_note1_element.when_present.click
    end
  end
  
  def go_clent_contacts
    go_sub_page 'Client Contacts'
  end
  
  def delete_client_contact
    message = self.alert do 
      self.delete_client_contact_link_element.when_present.click
    end
  end
  ...
  def delete_sale_notes
    message = self.alert do 
      self.delete_sale_note_link_element.when_present.click
    end  
  end
